...
Почему не одна таблица? Я решила, что этот момент нужно нормализовать, сделать словарь со значениями действий, чтобы избежать множественных повторений данных в основной таблице, упростить возможное расширение и сохранить целостность


### 3.  Исследование активности пользователей (на основе открытых данных VL.ru)
...
Эти коэффициенты внедрены в `generator.py` для симуляции реального поведения аудитории. Конечно может показаться притянуто за уши, но мне хотелось получить какие-то статистические данные для более правдоподобной активности.
